...
--
-- CARBON ISOTOPE TRACKING (C13/C14). After the runtime CN fluxes have updated
-- the bulk column carbon, this advances the column-total C13/C14 isotope ratios
-- with the real isotope physics (CNCIsoFluxMod / CNC14DecayMod): photosynthetic
-- discrimination on GPP, respiration removed at the bulk source ratio, and C14
-- radioactive decay. Because 'CLMState' carries no isotope-state field
-- (CLMDriver.hs is closed), the C13/C14 quantities are a ratio-diagnostic
-- derived from — and exactly conservative with — the bulk pools, rather than
-- independent prognostic pools. The diagnostic is then used to keep the bulk
-- carbon and its isotope-derived mass mutually consistent via
-- 'isotopeConsistentPool', which is the identity on physically consistent
-- pools. This block runs ONLY in the free-running runtime (clmCNActive); the
-- matched-state Fortran-parity harness is untouched.
cnBalanceCheckStep :: PhysicsStep
cnBalanceCheckStep _cfg ctx st
  | not (clmCNActive st) = st
  | otherwise = trackColumnCarbonIsotopes ctx st
